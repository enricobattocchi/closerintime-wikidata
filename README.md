...
- **next-intl** for i18n (6 locales: en, fr, de, es, it, pt)
...
- **Multilanguage** — available in English, French, German, Spanish, Italian, and Portuguese. Switch language in Settings; locale is reflected in the URL (`/fr/`, `/de/`, etc.).
...
- **Removable "Today" marker** — when you have 2+ events, you can hide the present-day marker and re-add it later.
...
src/
  ...
  components/
    ...
    ShareToolbar.tsx         # Copy link, share, export, zoom, edit-title, and show-Today buttons
    HelpModal.tsx           # Instructions modal
    SettingsModal.tsx       # Theme, timespan format, and language settings
    CategoryIcon.tsx        # Maps event type to MUI icon
  i18n/
    config.ts               # Locale list and names
    routing.ts              # next-intl routing config
    request.ts              # Server-side locale resolution
    messages/               # Translation JSON files (en, fr, de, es, it, pt)
  lib/
    wikidata.ts             # Wikidata REST API: search + fetch by Q-ID
    date-utils.ts           # UTC date creation, precise diffs, formatting
    event-label.ts          # Locale-aware event display names
...
- **Library tests** — date utils, timeline math, URL building, event labels
- **Component tests** — SettingsModal, ShareToolbar, TimelineMarker
- **i18n tests** — locale-aware date formatting, timespans, and event labels
...
- URLs like `/Q43653/Q107` or `/fr/Q43653/Q107` use Wikidata Q-IDs with an optional locale prefix. Append `~d` for death dates (e.g. `/Q42~d`). They are server-rendered with `generateMetadata()` so link previews show the timeline title and a dynamically generated OG image. The custom title and "hide Today" preference are stored as query params (`?t=...&now=0`).
