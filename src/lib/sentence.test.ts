...
describe("generateSentence", () => {
  ...
  describe("two events", () => {
    it("says 'closer to us' when gap between events > gap to now", () => {
      ...
      const events = [
        ...
        makeEvent({ id: "Q2", name: "Event B", year: 1990 }),
      ];
    ...
    it("says 'closer to [older]' when gap to now > gap between events", () => {
      ...
      const events = [
        makeEvent({ id: "Q1", name: "Event A", year: 1900 }),
        makeEvent({ id: "Q2", name: "Event B", year: 1910 }),
      ];
      const result = generateSentence(events);
      expect(result).toBe(
    ...
    it("says 'exactly halfway' when spans are equal", () => {
      ...
      const events = [
        makeEvent({ id: "Q1", name: "Event A", year: 2000 }),
        makeEvent({ id: "Q2", name: "Event B", year: 2012 }),
      ];
      const result = generateSentence(events);
      expect(result).toBe(
  ...
  describe("three events", () => {
    it("says 'more time between A and B' when first gap > last gap", () => {
      ...
      const events = [
        makeEvent({ id: "Q1", name: "Event A", year: 1800 }),
        makeEvent({ id: "Q2", name: "Event B", year: 1950 }),
        makeEvent({ id: "Q3", name: "Event C", year: 2010 }),
      ];
      const result = generateSentence(events);
      expect(result).toBe(
    ...
    it("says 'more time between C and us' when last gap > first gap", () => {
      ...
      const events = [
        makeEvent({ id: "Q1", name: "Event A", year: 2000 }),
        makeEvent({ id: "Q2", name: "Event B", year: 2005 }),
        makeEvent({ id: "Q3", name: "Event C", year: 2006 }),
      ];
      const result = generateSentence(events);
      expect(result).toBe(
    ...
    it("says 'same amount of time' when spans are equal", () => {
      ...
      const events = [
        makeEvent({ id: "Q1", name: "Event A", year: 2000 }),
        makeEvent({ id: "Q2", name: "Event B", year: 2012 }),
        makeEvent({ id: "Q3", name: "Event C", year: 2012 }),
      ];
      const result = generateSentence(events);
      expect(result).toBe(
  ...
  it("returns empty string for more than 3 events", () => {
    const events = [
      ...
      makeEvent({ id: "Q2", year: 1950 }),
      makeEvent({ id: "Q3", year: 1980 }),
      makeEvent({ id: "Q4", year: 2000 }),
    ];
